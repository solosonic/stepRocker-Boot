unit TMCLMsgBox;
(*  Projekt:  TMCL-Entwicklungsumgebung (IDE = Integrated Development Environment)

    Unit:     TMCLMsgBox
              Definiert die Funktion MessageDialog, MessageDialogPos und
              MessageDialogPosHelp. Diese leisten dasselbe wie die Delphi-Funktionen
              MsgDialog, MsgDialogPos und MsgDialogPosHelp, jedoch sind die Überschriften
              und Buttontexte auf Englisch. Ausserdem gibt es zusätzlich noch einen
              "Continue"-Button (wenn man mbContinue bei den Buttons angibt), der den
              Wert mrContinue (wird hier definiert) zurückliefert.

    Datum:    22. Juni 2004 OK
...
const
  mrContinue=mrYesToAll+1;

type
  TMsgDlgType = (mtWarning, mtError, mtInformation, mtConfirmation, mtCustom);
  TMsgDlgBtn = (mbYes, mbNo, mbOK, mbCancel, mbAbort, mbRetry, mbIgnore,
    mbAll, mbNoToAll, mbYesToAll, mbContinue, mbHelp);
...
implementation
uses ExtCtrls, Consts, Dlgs, Math;
...
var
  ...
  ButtonNames: array[TMsgDlgBtn] of string = (
    ...
    'YesToAll', 'Continue', 'Help');
  ButtonCaptions: array[TMsgDlgBtn] of Pointer = (
    @SMsgDlgYes, @SMsgDlgNo, @SMsgDlgOK, @SMsgDlgCancel, @SMsgDlgAbort,
    @SMsgDlgRetry, @SMsgDlgIgnore, @SMsgDlgAll, @SMsgDlgNoToAll, @SMsgDlgYesToAll,
    @SMsgDlgContinue, @SMsgDlgHelp);
  ModalResults: array[TMsgDlgBtn] of Integer = (
    mrYes, mrNo, mrOk, mrCancel, mrAbort, mrRetry, mrIgnore, mrAll, mrNoToAll,
    mrYesToAll, mrContinue, 0);
var
...
function MessageDialog(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint): Integer;
begin
  Result := MessageDialogPosHelp(Msg, DlgType, Buttons, HelpCtx, -1, -1, '');
end;

function MessageDialogPos(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer): Integer;
begin
  Result := MessageDialogPosHelp(Msg, DlgType, Buttons, HelpCtx, X, Y, '');
end;
...
begin
  with CreateMessageDialog(Msg, DlgType, Buttons) do
    try
      ...
      if X >= 0 then Left := X;
      if Y >= 0 then Top := Y;
      if (Y < 0) and (X < 0) then Position := poMainFormCenter;
      Result := ShowModal;
    finally
      Free;
    end;
