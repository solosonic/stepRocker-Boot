...
type
  ...
  TFlowControl = (fcNone, fcCTS, fcDTR, fcSoftware, fcDefault, fcRTSToggle);
  ...
  private
    ...
    procedure SetXonChar(Value: AnsiChar);
    procedure SetXoffChar(Value: AnsiChar);
  ...
  protected
    ...
    property EvtChar: AnsiChar read FEvtChar write FEvtChar default #0;
  ...
  public
    ...
    procedure GetCtrlStatus(var CtrlStatus:integer);
  end;
...
var
  ...
  HandlesToWaitFor  : TWOHandleArray;
...
procedure TCustomComm32.GetCtrlStatus(var CtrlStatus:integer);
begin
  GetCommModemStatus(FHandle,Cardinal(CtrlStatus));
end;
...
begin
  ...
  if not WriteFile(FHandle, Buf, Count, Cardinal(Result), @FWriteOS) then
...
begin
  ...
  if not ReadFile(FHandle, Buf, Count, Cardinal(Result), @OS) then Result := -1;
end;
...
procedure TCustomComm32.SetXonChar(Value: AnsiChar);
begin
  FXonChar := Value;
  if Enabled then
  begin
...
procedure TCustomComm32.SetXoffChar(Value: AnsiChar);
begin
  FXoffChar := Value;
  if Enabled then
  begin
  ...
  case FFlowControl of                      // if hDefault, nothing happens
    ...
    fcRTSToggle:
      begin
        DCB.Flags := DCB.Flags and (not fOutxCtsFlow);
        DCB.Flags := DCB.Flags and (not fRtsControl) or (RTS_CONTROL_ENABLE shl 12);
        DCB.Flags := DCB.Flags or (RTS_CONTROL_TOGGLE shl 12);
        DCB.Flags := DCB.Flags and (not fOutxDsrFlow);
        DCB.Flags := DCB.Flags and (not fDtrControl) or (DTR_CONTROL_ENABLE shl 4);
        DCB.Flags := DCB.Flags and (not fOutX) and (not fInX);
      end;
